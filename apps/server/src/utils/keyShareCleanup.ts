...
// Sweeps undeliverable E2E key shares (docs/e2e-dm-spec.md §12).
//
// Shares are normally claim-and-delete: a device drains its inbox and the rows
// disappear. Rows that are never claimed — a device that never comes back, or
// an account that was deleted (E2EKeyShare intentionally has no FK to User, so
// nothing cascades) — would otherwise live forever, holding routing metadata
// (who key-shared with whom, when, for which conversation) long past its use.
